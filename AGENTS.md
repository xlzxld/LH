...
> **版本 v2.1.2** | 本文件变更须走 Pull Request，并与同目录 `AUDIT-SPEC.md` / `BOOTSTRAP.md` 同批修订。
...
| 项 | 命令 / 取值 | 验证条件 |
|---|---|---|
| 技术栈 | Python ≥3.10（量化教程项目；依赖见 `requirements.txt`，虚拟环境 `.venv`） | — |
| 构建 (Build) | 无 | ✅ 无构建产物 |
| 测试 (Test) | `./.venv/Scripts/python.exe -m pytest code/tests/ -q` | ✅ 全绿；已知豁免清单内且非本次引入的失败不阻断，但须列出 |
| 静态检查 (Lint) | 无（未配置 lint 工具链；引入须走 PR） | ✅ 0 错误 0 警告（基线豁免同上） |
