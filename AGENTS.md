...
|---|---|---|
| 技术栈 | [自动填充] | — |
| 构建 (Build) | `[自动填充，无则写"无"]` | ✅ 退出码 0，产物正常生成 |
| 测试 (Test) | `[自动填充]` | ✅ 全绿；已知豁免清单内且非本次引入的失败不阻断，但须列出 |
| 静态检查 (Lint) | `[自动填充]` | ✅ 0 错误 0 警告（基线豁免同上） |
| 格式化 (Format) | `[自动填充 --check 等价命令]` | ✅ 对本次改动文件执行，0 差异 |
| 主干分支 | `[自动填充]` | ✅ 禁止未经 Pull Request 直接向主干提交 |
| 已知豁免清单 | `[自动填充，无则写"无"]` | 唯一合法的基线失败白名单，新增须走 PR；按 `命令:条目描述` 逐条登记，便于机器比对与复核 |
